...
import {
  ...
  sugerenciaAulas,
  sugerenciaNombreAulas,
} from "../../api/aulasDisponibles";
...
export const CreateBooking = () => {
  ...
  const getSugerenciasNombre = async () => {
    ...
    console.log(data);
    try {
      const response = await sugerenciaNombreAulas({
        fecha: data.fecha,
        nombreAula: name,
        periodos: periodoLista
      });
      if (response.libre == false || response == "") {
        alert("La aula indicada no existe o ya esta ocupada");
      }
      else {
        if (Array.isArray(response)) {
            setAulas(response);
          } else {
            setAulas([response.aula]);
          }
      }
    ...
    try {
      if (parseInt(estimado) > 10) {
        ...
        const response = await obtenerAulasDisponibles({
          ...
          periodos: periodoLista,
          capacidadMin:
            Math.floor(parseInt(estimado) / 100) != 0
              ? parseInt(estimado) / Math.floor(parseInt(estimado) / 100)
              : parseInt(estimado),
          capacidadMax:
            Math.floor(parseInt(estimado) / 100) != 0
              ? parseInt(estimado) / Math.floor(parseInt(estimado) / 100) + 50
              : parseInt(estimado) + 50,
        });
  ...
  return (
    <div>
      <div className={"create-booking-title"}>
        <ModalSuccess
          openModel={openModalS}
          handleOpen={handleOpenModalS}
          onSubmit={onSubmit}
          dataClassrooms={reserva}
        />
        <ModalWarning
          openModel={openModalW}
          handleOpen={handleOpenModalW}
        ...
        <div className={"form-title-column"}>
          ...
          <CommonText>
            A continuación indique la cantidad de estudiantes para la que
            necesita su reserva.
          </CommonText>
          <CommonText>
            El sistema le mostrara aulas basándose en la capacidad asignada, que
            sumen al total de la capasidad que necesite en caso sea necesario.
          </CommonText>
          <br />
          <CommonText>
            Si necesita un aula en especifico puede ingresarla, tenga en cuenta
            que si el aula que busca esta reservada por otro docente
          </CommonText>
          <CommonText>
            esta se mostrara como: ocupada o no disponible para una reserva.
          </CommonText>
        </div>
      </div>
      <div className={"table-top-header"}>
        <div className={"table-top-items"}>
          <div style={{ width: "30vw" }}>
            <CommonInput
              input={estimado}
              inputChange={setEstimado}
              label={"Indique la capacidad total que espera reservar"}
            />
            <CommonButton title={"Sugerirme Aulas"} onClick={getSugerencias} />
          </div>
          <div style={{ width: "30vw" }}>
            <CommonInput
              label={"Buscar un aula o area en especifico:"}
              input={name}
              inputChange={setName}
            />
            <CommonButton
              title={"Buscar por Nombre"}
              onClick={getSugerenciasNombre}
            />
          </div>
        </div>
        ...
        ? aulas.map((item, index) => {
            return (
              <div className={"table-suggest-item"}>
                ...
                <div className={"table-suggest-Fecha"}>
                  <ColoredTag>
                    {JSON.parse(sessionStorage.getItem("solicitud")).fecha}
                  </ColoredTag>
                </div>
                <div className={"table-suggest-Lugar"}>
